...
%         end
        ...
        for k = 1:size(dataVarMonthAve,1)
            ...
            [correlations.individual_pct(k,j,i),correlations.sig.individual_pct(k,j,i)] = ...
                corr(squeeze(dataVarMonthAve(k,[pct.highCl.ind.highind(k,:),pct.highCl.ind.lowind(k,:)],i,j))',...
                squeeze(toz_dataMonthArrange(k,inputs.tozmonth,[pct.highCl.ind.highind(k,:),pct.highCl.ind.lowind(k,:)])));
            for l = 1:size(dataVarMonth,3)
                ...
                [correlations.indmonths.individual_pct(k,l,j,i),correlations.indmonths.sig.individual_pct(k,l,j,i)] = ...
                    corr(squeeze(dataVarMonth(k,[pct.highCl.ind.highind(k,:),pct.highCl.ind.lowind(k,:)],l,i,j))',...
                    squeeze(toz_dataMonthArrange(k,inputs.tozmonth,[pct.highCl.ind.highind(k,:),pct.highCl.ind.lowind(k,:)])));
            end
        end
    end

function [GSS] = predruns_obsleaveoneout_test(data,waclon,waclat,inputs)
    ...
    precondtoz_pct = data.toz_zm_nodetrend([data.obspct.upperind,data.obspct.lowerind]);            
    ...
    for i = 1:length(allin)
        ...
        ozone_pct(1,i) = prctile(ozone_anomaly,20);
        ozone_pct(2,i) = prctile(ozone_anomaly,40);
        ozone_pct(3,i) = prctile(ozone_anomaly,60);
        ozone_pct(4,i) = prctile(ozone_anomaly,80);
        
        %Conditional shifts 
        ozone_left(i) = precondtoz(i) - ozone_ind_b(1).*i;
        
        
        if ozone_left(i) >= min(ozone_anomaly) && ozone_left(i) <= ozone_pct(1,i)
            pred_ind = 1;
            ozoneind = find(ozone_anomaly > min(ozone_anomaly) & ozone_anomaly <= ozone_pct(1,i));
        elseif ozone_left(i) > ozone_pct(1,i) && ozone_left(i) <= ozone_pct(2,i)
            pred_ind = 2;
            ozoneind = find(ozone_anomaly > ozone_pct(1,i) & ozone_anomaly <= ozone_pct(2,i));
        elseif ozone_left(i) > ozone_pct(2,i) && ozone_left(i) <= ozone_pct(3,i)
            pred_ind = 3;
            ozoneind = find(ozone_anomaly > ozone_pct(2,i) & ozone_anomaly <= ozone_pct(3,i));
        elseif ozone_left(i) > ozone_pct(3,i) && ozone_left(i) <= ozone_pct(4,i)
            pred_ind = 4;
            ozoneind = find(ozone_anomaly > ozone_pct(3,i) & ozone_anomaly <= ozone_pct(4,i));
        elseif ozone_left(i) > ozone_pct(4,i) && ozone_left(i) <= max(ozone_anomaly)
            pred_ind = 5;
            ozoneind = find(ozone_anomaly > ozone_pct(4,i) & ozone_anomaly <= max(ozone_anomaly));
        end
        
%         for pm = 1:5                     
%             if pm == 1
%                     conditionalshift_ind(i,:) = find(ozone_anomaly > min(ozone_anomaly) & ozone_anomaly <= ozone_pct(pm,i));
%             elseif pm > 1 && pm < 5
%                 conditionalshift_ind(i,:) = find(ozone_anomaly > ozone_pct(pm-1,i) & ozone_anomaly <= ozone_pct(pm,i));
%             else
%                 conditionalshift_ind(i,:) = find(ozone_anomaly > ozone_pct(pm-1,i) & ozone_anomaly <= mac(ozone_anomaly));
%             end
%             %condshift(i) = nanmean(ozone_anomaly) - nanmean(ozone_anomaly(conditionalshift_ind(i,:)));
%         end
...
%         ozone_anomaly = ozone_anomaly - nanmedian(ozone_anomaly);
        ...
        for j = 1:length(ERAdata.latitude)   
            ...
            temp_left(i,j,:) = squeeze(temp_left(i,j,:))' - nanmean(temp_anomaly);
...
%             tempdiff(i,j,:) = temp_anomaly_upper - temp_anomaly_lower;
            tempsignchange(i,j,:) = sign(nanmean(temp_anomaly) - nanmean(temp_anomaly(ozoneind,j,:))); % prediction
            
            %signchange(i,j,:) = sign(temp_anomaly_upper-temp_anomaly_lower);%-nanmedian(temp_anomaly)                        
            
            datasign(i,j,:) = sign(temp_left(i,j,:)); %actual data                        
            
            correct(i,j,:) = zeros(size(datasign(i,j,:)));
            
%              for l = 1:length(squeeze(datasign(i,j,:)))
%                 
%                 if tempsignchange(i,j,l) == 1
%                     pred(i,j,l) = 1;
%                 else
%                     pred(i,j,l) = -1;
%                 end              
...
%              end                          
     ...
     correct (tempsignchange == datasign) = 1;
     correct (tempsignchange ~= datasign) = 0;
...
%% plot
titles = {'Observed Heidke skill score (leave one out)','Observed 20^t^h percentile Heidke skill score (leave one out)','Observed percentile general skill score (leave one out)'};
...
%toplotp = permute(cat(1,p,p2),[1,3,2]);    
% subplotmaps(permute(cat(1,GSS.all.ind,GSS.pct.ind),[1,3,2]),longitudes,latitudes,{'seq','YlGnBu'},0,toplotp,16,titles,'Longitude','Latitude','HSS','on',...
%     [0,80],21,[longitudes(1:24:end)]-180,[longitudes(1:24:end)]-180,[0:15:90],[0:15:90],{''},1,[0 360],[0 90],0,'none',1,'Miller Cylindrical');         
...
titles = {'Observations','Observations 20^t^h percentiles','Ensemble member','Ensemble member 20^t^h percentiles'}; 

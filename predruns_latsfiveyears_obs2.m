...
for i = 1:1
    ...
    for j = 1:size(dataVarMonthAve,2)
        % detrending       
        if ifdetrend
            bdetrend(j,:,:) = predictors\squeeze(dataVarMonthAve(yearind,j,:));
            TSdetrended(:,j,:) =  squeeze(dataVarMonthAve(yearind,j,:)) - squeeze(bdetrend(j,1,:))'.*predictors(:,1) - squeeze(bdetrend(j,2,:))';
            TSleftdetrended(:,j,:) = squeeze(dataVarMonthAve(yearindleft,j,:)) - squeeze(bdetrend(j,1,:))'.*predictorsleft' - squeeze(bdetrend(j,2,:))';
        else
...
if plot_specific
    ...
    fig = figure;
    set(fig,'color','white','position',[100 100 1000 1200],'Visible','off')
    titles = {['No. ',sprintf('%02d',runs(1)),', Russia - 1995' char(8211) '2020'],...
        ['No. ',sprintf('%02d',runs(2)),', Russia - 1995' char(8211) '2020'],...
        ['No. ',sprintf('%02d',runs(3)),', Northern America - 1995' char(8211) '2020'],...
        ['No. ',sprintf('%02d',runs(4)),', Northern America - 1995' char(8211) '2020'],...
        ['No. ',sprintf('%02d',runs(5)),', Asia - 1995' char(8211) '2020'],...
        ['No. ',sprintf('%02d',runs(6)),', Asia - 1995' char(8211) '2020']};
    i = 1;
    for j = 1:6
    %     if  j < 4 
    %         toplotind = 1;
...
%     end
    ...
    %     end
        ...
        %yyaxis left
        plot(modelyears2(toplotind).m,toplot(toplotind).t(:,runs(j),i),'-o','LineWidth',3,'color',cbrew(1,:))
        hold on
        plot(modelyears2(toplotind).m,toplotmodel(toplotind).t(:,runs(j),i),'-o','LineWidth',3,'color',cbrew(2,:),'LineStyle','-')    
        set(gca,'Ycolor','k','fontsize',fsize-2);
        if j == 1 || j == 3 || j == 5
            ylabel('Temperture anomaly','fontsize',fsize);
        ...
        plot([years(2),years(2)],[-20,10],'Color','k','LineStyle','--');    
        plot([modelyears2(toplotind).m(1),modelyears2(toplotind).m(end)],[0,0],'Color','k','LineStyle','--');    
        forlim = [toplot(toplotind).t(:,runs(j),i);toplotmodel(toplotind).t(:,runs(j),i)]; 
        forlimsort = sort(forlim);
        ...
        annotation('textbox',get(sp(j),'position'),'String',['r = ', sprintf('%.2f',rval(toplotind,i,runs(j)))],...
    ...
    set(sp2,'position',[sppos(1,1) .025 sppos(1,3) .35]);
    caxis([-1 1]);
    xlabel('Longitude','fontsize',fsize+2);
    ylab = ylabel('Latitude','fontsize',fsize+2);
    set(ylab,'units','normalized','position',[-.09 .5 0]);
...
%     set(get(ch,'ylabel'),'string','r','fontsize',fsize+2)
    ...
    for i = 1:size(areas_lons,1)
        m_plot(lontoplot(runs(1),i),lattoplot(runs(1),i),'LineStyle','none','Marker',marks{i},...
            'MarkerSize',15,'MarkerFaceColor',cbrewqual2(i,:),'MarkerEdgeColor',cbrewqual3(i,:),'LineWidth',4)
    end

    toplotcorr = squeeze(corr2use(1,runs(2),:,:)); 
    toplotcorr = circshift(toplotcorr,size(toplotcorr,2)/2,2);
    ...
    m_coast('color','k','LineWidth',1);
    m_grid('ytick',0:30:90,'xtick',-180:60:180,'XaxisLocation','bottom','fontsize',fsize-2);            
    set(sp3,'position',[sppos(2,1) .025 sppos(2,3) .35]);
    caxis([-1 1]);
    xlabel('Longitude','fontsize',fsize+2);
    title(['No. ',sprintf('%02d',runs(2)),', correlations'],'fontsize',fsize+2);
    %ylab = ylabel('Latitude','fontsize',fsize+2);
    set(ylab,'units','normalized','position',[-.09 .5 0]);
    ch = colorbar;
    set(ch,'Position',[.89 .15 .0125 .1]);
    set(ch,'YTick',[-1:.4:1],'fontsize',fsize-2)%,
    ...
    for i = 1:size(areas_lons,1)
        m_plot(lontoplot(runs(2),i),lattoplot(runs(2),i),'LineStyle','none','Marker',marks{i},'MarkerSize',15,'MarkerFaceColor',cbrewqual2(i,:),'MarkerEdgeColor',cbrewqual3(i,:),'LineWidth',4)
    end
